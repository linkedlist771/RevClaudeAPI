...
if main_function == "API密钥管理":
    # API密钥管理部分
    api_key_function = st.sidebar.radio("API密钥管理", ["创建API密钥", "查看API密钥使用情况", "验证API密钥", "删除API密钥", "获取所有API密钥"])
    ...
    if api_key_function == "创建API密钥":
        ...
        top_n = st.number_input("请输入要显示的前几个API密钥", min_value=1, value=5, step=1)
            ...
            if response.status_code == 200:
                ...
                for api_key, info in tqdm(api_keys.items()):
                    try:
                        type = info["key_type"]
                        if type == key_type:
                            api_key_usage.append({"api_key": api_key, "usage": info["usage"]})
                    except Exception as e:
                        pass

                api_key_usage_df = pd.DataFrame(api_key_usage)
                
                api_key_usage_df = api_key_usage_df.sort_values("usage", ascending=False).head(top_n)
                chart = alt.Chart(api_key_usage_df).mark_bar().encode(
    ...
    # Cookie管理部分
    cookie_function = st.sidebar.radio("Cookie管理", ["上传Cookie", "删除Cookie", "刷新Cookie", "列出所有Cookie", "更新Cookie"])
